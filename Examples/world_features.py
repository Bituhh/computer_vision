...
def norm_pdf_multivariate(x, mu, sigma):
    # method for computing probability density of a multidimensional gaussian
    # inputs: gaussian parameters mu, sigma and test point x
    # output@ pdf for x
    size = len(x)
    x_mu = np.matrix(x - mu)
...
class contour:
    ...
    def __init__(self,input_contour,features):
        ...
        #self.new_feature_values = [eval(ft) for ft in features]
        
#        [axes_ratio, cconcavity_ratio, convexity_ratio, area_ratio, vertex_approx, length]
...
#        None
#
#class curvature_index(feature):
#      def __init__(self, feat_name):
#          super(curvature_index, self).__init__(feat_name)
#          
#      
...
#class object_view:
#    def __init__(self,key_letter,shape_name,col):
#        self.key = key_letter
...
class world:
    ...
    def __init__(self,folder,world_obj,sequence,features,logfile,colours,hsv_min,hsv_max):
    ...
    def classify(self,contour):
        ...
        for obj in range(len(self.world_experience)):
            # call to pdf function for computing...
            # providing ..., getting ...
            # l must be a list, x must be an array, gives a b/w image, use integer values
